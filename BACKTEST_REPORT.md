...
### Lot size comparison: 0.05 vs 0.1

`run_backtest_risk_capped.py` tests both the broker's real minimum lot (0.05) and double it (0.1) under the identical $100/$10-day-cap setup above. **Win rate and profit factor are identical between the two** — that's expected, not a coincidence: both are ratios (wins÷total, gross-win÷gross-loss) computed from the exact same sequence of trades, and lot size scales every trade's dollar P&L proportionally, so the ratios cancel out. Only the absolute dollar figures — net PnL, final balance, drawdown — actually change with lot size.

| Lot | Trades | Win % | PF | Net PnL | Final Balance | Max DD | Max DD $ | Worst single day | Cap tripped? |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 0.05 | 177 | 42.9% | 3.01 | +$197.74 | $297.78 | 4.87% | $7.31 | -$3.15 | Never (0/220 days) |
| **0.1** | 177 | 42.9% | 3.01 | **+$395.49** | **$495.56** | 8.44% | $14.61 | -$6.30 | Never (0/220 days) |

0.1 lot roughly doubles both the return and the drawdown, as expected from doubling position size on the same trades — and it's still safe relative to the $10/day cap (worst historical day was -$6.30, comfortably under the cap) and to the account itself (max ~8.4% drawdown on $100 is recoverable, not a blowup risk). This table is the actual persisted evidence for using 0.1 as the live default (see `trading_bot/live_engine.py`) — it was first computed in an ad-hoc chat comparison before being saved here as a proper, reproducible script + result file; if a future session or reader only checked the repo's history before this was added, they'd have had no way to find that 0.1 was ever genuinely tested. Re-run `python trading_bot/run_backtest_risk_capped.py M15` any time to reproduce this table from scratch.
...
**Update, 2026-09-14: items (1)-(3) above are now done** — see [Parameter optimization](#parameter-optimization-2026-09-14) for the tuning pass and [Lot size comparison](#lot-size-comparison-005-vs-01) for the sizing decision. The resulting config (M15, tuned parameters, 0.1 fixed lot, $10/day loss cap, profit uncapped) is this repo's best-validated result to date — frozen and importable from `trading_bot/strategy_presets.py` (`VALIDATED_M15_BASELINE_2026_09_14`) as a reference/rollback point, independent of wherever `StrategyParameters()`'s live defaults drift to in future tuning passes, with its exact values locked in by `test_strategy_presets.py`. Item (4)'s concern about thin OOS samples still applies - 61 OOS trades is better than 3-42 but still not large - and this remains backtest evidence, not forward-tested proof; see [Honest caveats](#honest-caveats) below, which is unchanged by any of this.
