"""
SQLite Persistence Layer for the NASDAQ Order-Flow Scalper.
Persists settings, closed trades, and logs across restarts.

Same schema/shape as the sibling gold bot's storage.py (proven pattern),
plus a `model` column recording which of the 3 playbooks (AAA / SQUEEZE /
FAILED_AUCTION) triggered each trade.
"""
...
class BotStorage:
    ...
    def __init__(self, db_path: str = "nasdaq_trades.sqlite"):
    ...
    def save_trade(self, trade_data: Dict[str, Any]) -> int:
        ...
        with self._get_connection() as conn:
            ...
            """, (
                ...
                trade_data.get("lot_size") or trade_data.get("volume", 0.05),
                ...
                trade_data.get("magic_number", 9312001),
                now
            ))
            conn.commit()
    ...
    def update_closed_trade(self, ticket: int, exit_price: float, net_pnl: float, exit_reason: str = "SL/TP Hit"):
        ...
        with self._get_connection() as conn:
            ...
            cursor.execute("""
                UPDATE trades
                SET exit_price = ?, net_pnl_usd = ?, exit_reason = ?, exit_time = ?
                WHERE ticket = ?
